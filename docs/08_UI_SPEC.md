...
Dashboard Shell
  ...
  ├─ Result Mode
  │   └─ Active Layer analysis screen
  ├─ Collection Mode
  │   └─ Active Layer collection screen: 수집원 요약, 상세, 레이어 상태, 데이터 정합성
  ├─ Layer 1: 흐름
  ...
  ├─ Layer 3: 리더십 상세
  │   ├─ Top Strip: RRG 경로 selector와 현재 RS 리더 / 모멘텀 선두 flow를 가로 배치
  ...
  ├─ Layer 4: 검증 Lab
  │   ├─ Compact Overview: 데이터 수집 / Replay / 패턴 진단 / 표본 관측치
  │   ├─ Current Pattern Guardrails: 현재 리더/후보/주의 패턴의 검증 연결
  │   ├─ Replay Status: 30D / 90D / 180D 가능 범위를 Replay 상태 안에 병합
  │   ├─ Pattern Diagnostics: rulebook pattern별 historical diagnostics matrix
...
### 3.1.1 Result / Collection Surface

각 Layer 탭은 `결과 / 수집` 전환을 갖습니다. 수집 화면은 별도 1차 탭이 아니라 현재 Layer 탭 안에서 결과 화면과 교체됩니다.

```text
Layer 1 + 결과: 흐름 분석 화면
Layer 1 + 수집: Layer 1 수집원/정합성 화면
Layer 2 + 결과: 여력 분석 화면
Layer 2 + 수집: Layer 2 수집원/정합성 화면
Layer 3 + 결과: 리더십 분석 화면
Layer 3 + 수집: Layer 3 수집원/정합성 화면
Layer 4 + 결과: 검증 Lab 화면
Layer 4 + 수집: Layer 4 수집원/정합성 화면
```

### 3.1.2 Layer Data Context Section

수집 화면은 `FreshnessBar`, `ContextRail`, `DataQualityStrip`을 `레이어 수집 데이터` 섹션 안에 통합합니다. 리포트 문장은 결과 화면 또는 내부 view model에서만 사용하고 수집 화면의 주요 정보로 반복하지 않습니다.

```text
LayerDataContextSection
  source summary/detail: active layer source rows
  status rail: active layer context rail
  data quality: current layer completeness and issues
```

Scope:

```text
Layer 1: SPY, QQQ, RSP, IWM, ^VIX helper, market tape, breadth proxy
Layer 2: Yahoo sector prices, FRED context, participation, risk trigger
Layer 3: sector snapshots, RS/RRG, leadership reconciliation
Layer 4: sector history, validation API, replay coverage, context/FRED coverage
```

이 섹션은 판단 화면보다 위에 있으나, 판단 카드와 중복되는 별도 source/detail 카드를 만들지 않습니다.
...
API가 `leadership_reconciliation`을 제공하면 Layer 3 top strip은 이 narrative를 우선 사용한다. 제공되지 않는 환경에서는 기존 current RS leader와 momentum leader 비교로 fallback한다.
...
Layer 3 상단은 RRG 경로 selector와 리더십 flow를 같은 가로 스트립에 배치해 빈 공간을 줄인다. 기본 상세 선택은 Layer 1의 현재 RS 리더를 따른다. 좌측 rail은 모멘텀 선두 후보를 보여주므로, 선택 상세와 rail 1위가 다를 수 있다. 이 불일치는 오류가 아니라 전환 신호로 표시한다.
...
Top:
  - 컴팩트 검증 요약 스트립
  - 데이터 수집 / Replay / 패턴 진단 / 표본 관측치를 낮은 상태 스트립 안에서 표시
...
Bottom:
  ...
  - 패턴명, 표본, 표본 관측, 20D 이후, 60D 이후, 20D 하락, 신뢰도를 heat cell과 중심축 점으로 가로 배치
  - 데이터 제한이 있을 때만 제한 카드
```
...
Layer 4는 전역 `expose_probability`만 보고 숫자를 표시하지 않는다. 각 `pattern_diagnostics[].status`가 `ready`일 때만 표본 관측 확률을 표시하고, `thin_sample` 또는 `collecting`이면 `표본 부족`, `forward label 대기`, `라벨 커버리지 낮음` 같은 readiness copy를 표시한다.
...
이 정보는 각 Layer의 `수집` 화면 안에서 `LayerDataContextSection`, `FreshnessBar`, `ContextRail`로 표시한다. 활성 탭과 무관한 수집원은 현재 Layer의 수집 화면에 섞지 않는다.

`DataQualityStrip`은 같은 섹션 안에서 freshness와 별개로 레이어별 정합성만 표시한다.

```text
Layer 1: sector snapshot, SPY/QQQ/RSP/IWM/^VIX helper, proxy breadth warnings
Layer 2: official FRED context 4종, stale context rows
Layer 3: sector panel count, missing RS Momentum, module warning count
Layer 4: /api/history complete sector days, /api/validation ready/thin pattern count, endpoint as_of mismatch
```

Layer 4의 정합성 정보는 `현재 진행` 카드와 `패턴별 이력 진단 결과` 사이에 별도 카드로 렌더링하지 않는다. Layer 4의 `수집` 화면으로 분리해 중간 공간을 낭비하지 않는다.
...
frontend/src/features/radar/components/DataCollectionView.tsx: 현재 Layer의 수집 화면 본체
frontend/src/features/radar/components/LayerDataContextSection.tsx: 수집원/상태/정합성 통합 섹션
